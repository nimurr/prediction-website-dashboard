import { useState } from "react";
import {
    Card,
    Typography,
    Tag,
    Button,
    Space,
    Tooltip,
    Popconfirm,
    Modal,
    message,
    Divider,
} from "antd";
import { AiOutlineEye } from "react-icons/ai";
import { useParams } from "react-router-dom";
import { useDeclearWinerMutation, useGetFullDetailsPricePredictionQuery } from "../../../redux/features/pricePrediction/pricePrediction";
import { imageBaseUrl } from "../../../config/imageBaseUrl";
import moment from "moment";

const { Text, Paragraph, Title } = Typography;

const AddPricePredictionUserDetails = () => {


    const { id } = useParams();
    const predictionId = new URLSearchParams(window.location.search).get("predictionId");

    const { data } = useGetFullDetailsPricePredictionQuery({ userId: id, predictionId: predictionId });
    const predictionInfo = data?.data?.predictionInfo;
    const userInfo = data?.data?.userInfo[0];

    console.log(userInfo);



    // Predictor details (from your test data)
    const [status, setStatus] = useState("Pending");
    const details = {
        btctalk: "CryptoKing007",
        btcAddress: "1BoatSLRHtKNngkdXEeobR76b53LETtpyT",
        casinoUser: "LuckySpin777",
        email: "luckygamer777@example.com",
        team: "Barcelona",
        price: 150,
        description:
            "Argentina will dominate possession and secure a 3-1 victory over Brazil in the upcoming Copa America final, with Messi scoring at least once.",
    };

    const [previewOpen, setPreviewOpen] = useState(false);

    const Row = ({ label, children }) => (
        <div className="flex flex-col md:flex-row md:items-center md:gap-6 py-3">
            <div className="md:w-64 w-full text-gray-600 font-medium">{label}</div>
            <div className="flex-1">{children}</div>
        </div>
    );

    const [updateDeclareWinner] = useDeclearWinerMutation();

    const handleDeclareWinner = async () => {
        // setStatus("Winner");
        try {
            const result = await updateDeclareWinner({ userId: id, predictionId: predictionId });
            console.log(result);
            if (result?.data?.code == 200) {
                message.success("Declared as Winner");
            }
            if (result.error) {
                message.error(result?.error?.data?.message || "Failed to declare as winner");
                throw new Error(result.error);
            }
            // setStatus("Winner");
        } catch (error) {
            console.log(error);
        }
    };

    return (
        <section className="py-5 px-3">
            {/* Header / Match info */}
            <Card bordered className="mb-6 border-2 p-5 bg-gradient-to-tr to-purple-400 rounded-lg from-white border-[#704AAA]">
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div>
                        <Title level={3} style={{ margin: 0 }}>
                            {predictionInfo?.bitcoinTitle}
                        </Title>
                        <h2 className="text-4xl flex items-center gap-5 my-3">
                            <img className="w-10 " src={imageBaseUrl + predictionInfo?.bitcoinImage} alt="" />
                            {/* {header.sportLine} */}
                        </h2>
                        <div className="text-sm text-gray-600">{moment(predictionInfo?.createdAt).format("DD-MM-YYYY HH:mm A")}</div>
                        <Paragraph style={{ marginTop: 12, marginBottom: 0 }}>
                            {predictionInfo?.bitcoinSubtitle}
                        </Paragraph>
                    </div>


                </div>
            </Card>

            {/* Details (NOT a table) */}
            <Card
                bordered
                className="rounded-xl text-xl bg-[#704aaa51]"
                styles={{ body: { paddingTop: 8, paddingBottom: 8 } }}
            >
                <div className="px-1">
                    <div>
                        <img className="w-48 " src="/Sport/users-image.png" alt="" />
                    </div>
                    <Row label="Bitcointalk Username">
                        <Text className="text-xl">{userInfo?.bitcointalkUsername}</Text>
                    </Row>
                    <Divider className="my-0" />
                    <Row label="Bitcoin Predict Price">
                        {userInfo?.predictedPrice}$
                    </Row>
                    <Row label="Bitcoin Address">
                        <Text copyable className="text-xl font-mono break-all">
                            {userInfo?.bitcoinAddress}
                        </Text>
                    </Row>
                    <Divider className="my-0" />
                    <Row label="Casino Username">
                        <Text className="text-xl">{userInfo?.casinoUsername}</Text>
                    </Row>
                    <Divider className="my-0" />
                    <Row label="Email (optional)">
                        <a href={`mailto:${userInfo?.email}`} className="break-all">
                            {userInfo?.email}
                        </a>
                    </Row>
                    <Divider className="my-0" />

                    <Row label="Winner Status">
                        <Text className={userInfo?.isWinner ? "bg-green-600 text-white px-2 py-1 rounded-lg" : "bg-red-600 text-white px-2 py-1 rounded-lg"}>{userInfo?.isWinner ? "Winner" : "Pending"}</Text>
                    </Row>

                    <Divider className="my-0" />
                    {/* <Row label="Prediction Description">
                        <Paragraph className="text-xl" style={{ marginBottom: 0, whiteSpace: "pre-line" }}>
                            {details.description}
                        </Paragraph>
                    </Row> */}
                    <Space>
                        <button onClick={handleDeclareWinner} className="bg-[#704AAA] text-white py-3 px-8 rounded-lg">Declare As Winner</button>
                    </Space>
                </div>
            </Card>


        </section>
    );
};

export default AddPricePredictionUserDetails;
