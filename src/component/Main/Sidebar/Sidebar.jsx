...
import { RiBtcLine, RiMoneyDollarCircleFill } from "react-icons/ri";
import { FaBriefcase, FaCopy, FaUserAlt, FaUsers } from "react-icons/fa";
import { MdDashboard, MdOutlineContactPage, MdSportsSoccer } from "react-icons/md";
...
const sidebarItems = [
  ...
  },
  {
    ...
    name: "Add Score Predictions",
    icon: <MdSportsSoccer className="size-6" />
  },
  {
    path: "/add-price-prediction",
    name: "Add Price prediction",
    icon: <RiBtcLine className="size-6" />
  },
  {
    path: "/add-poker-tournament",
    name: "Add Poker Tournament",
    icon: <GiCardBurn className="size-6" />
  },
  {
    path: "/reviews-all",
    name: "Add Reviews",
...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <div>
      {/* Desktop Sidebar */}
      <div className="hidden overflow-y-auto md:block w-full md:w-[200px] lg:w-[250px] xl:w-[280px] h-full bg-[#704AAA] fixed shadow-2xl">
        <Link to={"/"} className="flex flex-col justify-center items-center pt-5 gap-2 mb-10 text-black">
          <img src={logo} alt="logo" className="w-[100px] my-5 " />
        </Link>
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed top-0 overflow-y-auto left-0 z-40 w-64 h-full bg-[#704AAA] shadow-lg transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        ...
        <ul className="flex flex-col gap-3 mt-10">
          {sidebarItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `w-[70%] mx-auto px-5 py-2 flex items-center gap-3 text-white ${isActive ? "bg-[#704AAA] " : ""
                }`
              }
            >
              {item?.icon}
